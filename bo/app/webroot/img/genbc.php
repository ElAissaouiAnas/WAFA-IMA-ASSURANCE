...
$thumb = imagecreatetruecolor(500, 40);
...
for($i = 0; $i < strlen($TheString); $i++)
{
    ...
    //récupération de l'index du caractère
...
for($x=0; $x < strlen($xCodeBar); $x++)
{
    if($xCodeBar[$x] == "1")
    {
        imageline($thumb, $x_ref, 0, $x_ref, 40, $c_b);
    }
    else
    {
        imageline($thumb, $x_ref, 0, $x_ref, 40, $c_w);
    }
    $x_ref++;
}
...
$IMG = imagecreatetruecolor($x_ref, 40);
imagefill($IMG, 0, 0, $fond);
imagecopymerge ($IMG, $thumb, 0, 0, 0, 0, 500, 40, 100 );
//------------------------------------------------------------------------------
